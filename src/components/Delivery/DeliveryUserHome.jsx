...
const DeliveryUserHome = ({ name, userId }) => {
    ...
    const fetchDeliveredOrders = async () => {
      try {
        const response = await axiosInstance.get(`/orders/delivery-orders`, {
          params: { delivery_id: _id }
        });

        if (response.data.success) {
          const completedOrders = response.data.data.filter(order => order.status === "Delivered");
          setDeliveredOrders(completedOrders);
        } else {
          toast.error(response.data.message || "Failed to fetch delivered orders");
        }
      } catch (error) {
  ...
  return (
    <div className={`flex flex-col  ${theme === 'dark' ? ' text-white' : 'text-gray-800'}`}>
              ...
              <table className="min-w-full divide-y divide-gray-700">
                ...
                <tbody className={`divide-y ${theme === 'dark' ? 'divide-gray-700' : 'divide-gray-200'}`}>
                  {deliveredOrders.map((order) => (
                    <tr key={order._id || "unknown"}>
                      <td className="px-4 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          <div>
                            <div className="font-medium">{order.restaurant?.name || "N/A"}</div>
